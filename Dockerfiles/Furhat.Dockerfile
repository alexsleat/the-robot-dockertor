...
# Install Furhat
#-------------------------------------------------------------------------
RUN wget -P /opt/furhat/ https://s3-eu-west-1.amazonaws.com/furhat-files/launcher/1.2.7/furhat-sdk-desktop-launcher.AppImage
RUN chmod +x /opt/furhat/furhat-sdk-desktop-launcher.AppImage
## This needs to be run in run time, can't get it to build with the gui popup that's required
# RUN (cd /opt/furhat/ && ./furhat-sdk-desktop-launcher.AppImage --appimage-extract-and-run --no-sandbox)
#-------------------------------------------------------------------------
